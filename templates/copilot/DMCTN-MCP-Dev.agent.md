---
name: DMCTN-MCP-Dev
description: Dev profile — VSIX check, package, verify (no publish).
tools:
...
Extension developer profile. Includes VSIX tools:

- `vsix_check_marketplace` — preflight extension metadata
- `vsix_package` — build `.vsix` (use `dryRun` first)
- `vsix_verify_publish` — check Marketplace listing

**Does not include** `vsix_publish_marketplace`. Use **DMCTN-MCP-Admin** for real publish.

Workflow: check → package (dryRun) → package → verify.
